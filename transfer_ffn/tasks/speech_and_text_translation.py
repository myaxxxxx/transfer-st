...
from transfer.datasets.speech_and_text_translation_dataset import (
...
@register_task("speech_and_text_translation_multi")
class SpeechAndTextTranslationTaskMulti(LegacyFairseqTask):
    @classmethod
    def add_args(cls, parser):
        ...
        parser.add_argument(
            "--st-training",
            action="store_true",
            help="speech translation training"
        )
        parser.add_argument(
            "--ext-mt-training",
            action="store_true",
    ...
    def load_dataset(self, split, epoch=1, combine=False, **kwargs):
        ...
        if self.args.st_training:
            ...
            concat_dataset.append(ModalityDatasetItem(
                "st",
                st_dataset,
                [self.args.max_audio_positions, self.args.max_source_positions, self.args.max_target_positions],
    ...
    def load_st_dataset(self, split, epoch):
        is_train_split = split.startswith("train")
        return SpeechAndTextTranslationDatasetCreator.from_tsv(
            self.args.data,
            self.data_cfg,
            split,
            self.tgt_dict,
    ...
    def max_positions(self):
        return self.args.max_audio_positions, self.args.max_source_positions, self.args.max_target_positions

    def build_model(self, args):
        args.input_feat_per_channel = self.data_cfg.input_feat_per_channel
        args.input_channels = self.data_cfg.input_channels
        args.speaker_to_id = self.speaker_to_id
        model = super(SpeechAndTextTranslationTaskMulti, self).build_model(args)
    ...
    def build_generator(
        ...
        if getattr(self.args, "debug_task", "st") == "st":
